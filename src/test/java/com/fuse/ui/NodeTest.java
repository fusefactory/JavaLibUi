...
public class NodeTest {
  ...
  @Test public void addOnTop(){
    Node scene = new Node();
    scene.setPlane(11.0f);
    Node layer1 = new Node();
    assertEquals(layer1.getPlane(), 0.0f, 0.000001f);
    scene.removeChild(layer1);

    // add normal
    scene.addChild(layer1);
    assertEquals(layer1.getPlane(), 0.0f, 0.000001f); // plane not affected
    scene.removeChild(layer1);

    // addOnTop
    scene.addOnTop(layer1);
    assertEquals(layer1.getPlane(), 12.0f, 0.000001f); // plane not affected
    scene.removeChild(layer1);

    // with optional additional amount to raise
    scene.addOnTop(layer1, 10.0f);
    assertEquals(layer1.getPlane(), 21.0f, 0.000001f); // plane not affected

    Node layer2 = new Node();
